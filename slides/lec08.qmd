...
* model local variation $e(s_0)$ as a weighted linear combination of the local variation at the known points $\lambda (Z-\mu)$, then add the mean:
$\hat{Z}(s_0) = \mu + \lambda (Z-\mu)$ <br> with $\lambda = v' V^{-1}$ (see last lecture)



[**Ordinary Kriging:**]{.green}

* assume the global mean $\mu$ to be constant but _unknown_
...
We also saw what to do when the mean is _varying_ but _known_:

* this is simple kriging with a vector of known means
 $\mu(s) = (\mu(s_1),\mu(s_2),...,\mu(s_n))'$.
...
* known, but varying mean: Simple Kriging 
 with mean  $\mu(s) = (\mu(s_1),\mu(s_2),...,\mu(s_n))'$. 
...
interested in predicting the average of $Z(s)$ over a block, $B_0$, i.e.
