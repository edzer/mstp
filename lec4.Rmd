...
(location on the map)
* we have $x$ and $y$ \pause
* we may have land use data \pause
* we may have soil type or geological data \pause
* we may have remote sensing imagery \pause
...
* both deal with {\em two} continuous variables
* both look at (first order) {\em linear} relations
* when correlation is significant, the regression {\em slope} is significant
Differences:
* Regression distinguishes $y$ from $x$: $y$ depends on $x$, not reverse;
* the line $y=ax+b$ is {\em not} equal to the line $x = cx + d$
* Correlation is symmetric: $Cor(x,y)=Cor(y,c)$
* Correlation coefficient is unitless and within $[-1,1]$, 
regression coeficients have data units
* Regression is concerned with {\em prediction} of $y$ from $x$.
...
* to which extent is regression {\em robust} agains a violated assumption 
of independent observations?
* to which extent {\em is} the assumption violated? (how strong is the
correlation)
