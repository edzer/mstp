...
-   *univariate*: If $Z$ follows a normal distribution, its probability distribution is *completely* characterized by its mean $E(Z)=\mu$ and variance ${\rm Var}(Z)=\sigma^2$
-   *multivariate*: If the vector $Z=(Z_1,Z_2,...,Z_p)$ follows a *multivariate* normal distribution, its marginal distributions are univariate normal, and its *joint* probability distribution is *completely* characterized by the mean vector $E(Z)=\mu=(\mu_1,...\mu_p)$ and covariance matrix $V$, of which element $(i,j)$ equals ${\rm Cov}(Z_i,Z_j)$
...
Given prediction location $s_0$, and data locations $s_1$ and $s_2$, we need: ${\rm Var}(Z(s_0))$, ${\rm Var}(Z(s_1))$, ${\rm Var}(Z(s_2))$, ${\rm Cov}(Z(s_0),Z(s_1))$, ${\rm Cov}(Z(s_0),Z(s_2))$, ${\rm Cov}(Z(s_1),Z(s_2))$.

How to get these covariances?

-   given a single measurement $z(s_1)$, we can not infer ${\rm Var}(Z(s_1))$
-   given two measurements $z(s_1)$ and $z(s_2)$, we can *never* infer ${\rm Cov}(Z(s_1),Z(s_2))$
-   given a time series at $s_1$ and $s_2$, we can infer
-   ${\rm Cov}(Z(s_1),Z(s_2))$, but how to infer
-   ${\rm Cov}(Z(s_0),Z(s_1))$ and
-   ${\rm Cov}(Z(s_0),Z(s_2))$?
...
-   *mean* ${\rm E}(Z(s_1)) = {\rm E}(Z(s_2)) = ... = m$
-   *variance* ${\rm Var}(Z(s_1)) = {\rm Var}(Z(s_2)) = ... = \sigma^2_0$
-   *covariance* ${\rm Cov}(Z(s_1),Z(s_2)) = {\rm Cov}(Z(s_3),Z(s_4))$ if $s_1-s_2=s_3-s_4$: distance/direction dependence

Second order stationarity: ${\rm Cov}(Z(s),Z(s+h)) = C(h)$

which implies: ${\rm Cov}(Z(s),Z(s)) = {\rm Var}(Z(s))= C(0)$
