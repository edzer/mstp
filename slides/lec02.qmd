...
+ the choice of components and how we try to model them depends on what we know about the process
   + say we know that daily mean temperature (at a certain location) follows a seasonal periodicity, so we could try to model this
component with a periodic function of time $f(t)$, like a sinus function
