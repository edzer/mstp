...
where $f_Z(\cdot)$ is the probability _density_ function of $Z$. The
...
$$=\lambda'V\lambda + \sigma_0^2 - 2 \sum_{i=1}^n \lambda_i \Cov(Z(s_i),Z(s_0)) $$
...
meuse.grid$sk = sk(meuse, meuse.grid, mean(log(meuse$zinc)), function(h) exp(-h/300))
spplot(meuse.grid["sk"], main = "simple kriging", scales = list(draw = TRUE))
```
