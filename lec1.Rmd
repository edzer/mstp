...
ls()
names(meteo)
plot(T.outside~date, meteo, type='l', ylab = "Temperature, deg C", xlab = "date, 2007")
...
### 4.4 Autocorrelation
Autocorrelation (or lagged correlation) is the correlation between $y_i$ and $y_{i+h}$,
with $h$ the lag:
$$
...
+ AR(1) have ``zero'' _partial_ autocorrelations beyond lag 1
...
etc. In the limit, we may write any AR process as an (infinite) MA process,
and vice versa.
