...
-   Mathematical models for spatiotemporal processes can be controlled by parameters. Given a particular model and a limited number of process observations (data), we want to infer [*best fitting*]{.green} values for the parameters.

. . .

-   To define [*best fitting*]{.green}, we need to setup an *objective function* which takes parameters as arguments and yields a single scalar measure that reflects how well (or usually how badly) specific parameter values fit the the data.
...
but might be more complex. In this course, we will concentrate on the [*sum of squared residuals*]{.green}. Notice, however, that the complexity of $G$ depends also on the model itself, which is evaluated for each observation. Let us start with linear models.
...
1. determine an interval $[a,b]$, in which the minimum is expected to lie  
2. determine two points $c$ and $d$ within this interval splitting it according to golden ratio
3. evaluate the function at these two points, compare $f(c)$ and $f(d)$ 
4. depending on which value is lower, update your interval to $[a,d]$ (if $f(c)$ is smaller) or to $[c,b]$ (if $f(d)$ is smaller). Continue with the new interval, repeat until we're sufficiently close (in terms of our goal, or of numerical resolution)
...
- our new search interval is now betewen $a$ and $d$ (so $d$ is the new $b$)
...
Again, $f(c)$ is greater than $f(d)$ and the minimum must be located between $c$ and $b$.
...
## Combined linear and golden search - approach:
...
## Side note: Gauss-Newton and the Normal equations
