...
-   Example: Approximate $\pi$ by counting the portion of $(U(-1,1), U(-1,1))$ distributed random points that lie inside the unit circle.

## MCMC - Markov chain Monte Carlo methods

**Monte Carlo algorithms:**

Example: Approximate $\pi$ by counting the portion of $(U(-1,1), U(-1,1))$ distributed random points that lie inside the unit circle ? Let´s try this out.
...
$\text{Area of the square} = \text{side} \times \text{side} = 2 \times 2 = 4$

$\text{Area of the circle} = \pi \times r^2 = \pi \times (1)^2 = \pi \times 1 = \pi$

$\text{proportion_square_circle} = \frac{\text{Area of the circle}}{\text{Area of the square}} = \frac{\pi}{4}$

$\pi = \text{proportion_square_circle} \times 4$
...
y <- runif(n_samples,min= -1, max = 1)

# Plot the square and points
plot(x, y, xlim = c(-1, 1), ylim = c(-1, 1), main = "Monte Carlo Estimation of Pi", xlab = "x", ylab = "y", 
     col = ifelse(x^2 + y^2 <= 1^2, "red", "blue"), pch = 20, asp = 1)
grid()

# Add a circle
symbols(0, 0, circles = 1, inches = FALSE, add = TRUE, fg = "black")
...
monte_carlo_pi <- function(n_samples){
  ...
  x <- runif(n_samples, min=-1,max = 1)  # Uniformly distributed random numbers for x-coordinate
  y <- runif(n_samples,min=-1,max = 1)  # Uniformly distributed random numbers for y-coordinate
  # Calculate the number of points that lie inside the circle
  inside_circle <- sum(x^2 + y^2 <= 1^2)
  
  # Estimate Pi as shown in previous slide
  pi_estimate <- 4 * (inside_circle / n_samples)
  # Print the estimate and difference to actual Pi
  print(pi_estimate)
