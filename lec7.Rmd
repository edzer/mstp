...
calc_beta = function(X, V, y) {
	XtVinv = t(solve(V, X))
	solve(XtVinv %*% X, XtVinv %*% y)
}
...
uk = function(data, newdata, X, x0, cov, beta) {
	...
	y = data[[1]]
	if (missing(beta))
		beta = calc_beta(X, V, y)
	mu = X %*% beta
	x0 %*% beta + t(v) %*% solve(V, y - mu)
}
...
x0 = matrix(1, length(newdata), 1)
y = uk(data, newdata, X, x0, cov) # mu unknown
plot(coordinates(newdata)[,1], y, type='l', ylim = c(1,5))
...
y = uk(data, newdata, X, x0, cov) # mu unknown
plot(coordinates(newdata)[,1], y, type='l')
points(as(data, "data.frame")[,c(2,1)], col='red', pch=16)
beta = calc_beta(X, cov(spDists(data)), data[[1]])
beta
